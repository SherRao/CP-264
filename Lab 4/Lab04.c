...
int main(int argc, char *argv[]) {
   ...
   char *p_text[MAX_NUM_LINES]; // Array of pointers to "char" - Have a total number of MAX_NUM_LINES pointers.
   char filename_in[] = "text.txt"; // Input file name.
   FILE *fp_in = NULL; // Input file pointer.
   return_val = 0; // Assume all is well.

   // Initialize "msg" message buffer by filling entire buffer with NULLs.
   for(int i=0; i < MAX_MSG_NUM; i++)
      for(int j=0; j < MAX_MSG_LNG; j++)
         msg[i][j] = cNUL;
   ...
   if((fp_in = fopen(filename_in, "r")) == NULL) { // Open input file and check if open is successful.
      ...
      if((nlines = read_text(p_text, fp_in)) >= 0 ) {  // Read the entire input text in the "p_text" array of char pointers.
...
// Takes an array of pointers to char char and the input file pointer.
int read_text(char *p_text[], FILE *fp) {
   //===========================
   ...
   char [MAX_LINE_LNG]; // Local storage for the current line of text just read.
